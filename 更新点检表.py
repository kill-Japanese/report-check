...
// ==================== 协作模式（多人在线编辑） ====================
// 【关键修复】必须在 syncFromExcel() 调用之前声明，否则 TDZ 错误
...
function isProjectArchived(p) {
  ...
  // 协作模式：只使用服务器返回的数据
  if (collabIsEnabled()) {
    return p['已归档'] === true || p['已归档'] === '已归档';
  }
  // 非协作模式：使用本地 archived 对象
  const pid = typeof p.id === 'number' ? p.id : parseInt(p.id);
  if (archived[pid] || archived[p.id]) return true;
  if (p['已归档']) return true;
  return false;
}
...
async function callActionApi(endpoint, data) {
  ...
  try {
    ...
    if (result.success && result.allProjects) {
      ...
      // 重建部门索引
      const depts = {};
      result.allProjects.forEach(function(p) {
        if (p['已归档']) return;
        const d = p['部门'] || '未分配';
        if (!depts[d]) depts[d] = [];
        depts[d].push(p);
      ...
      RAW_DATA.depts = depts;
      // 更新统计
      const active = result.allProjects.filter(p => !p['已归档']);
      RAW_DATA.stats = RAW_DATA.stats || {};
      RAW_DATA.stats.total = active.length;
    }
    return result;
