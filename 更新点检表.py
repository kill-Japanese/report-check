...
<div class="container">
  ...
  <div class="update-tip">
    ...
    |
    方式B：点击下方「✏️ 编辑模式」直接在页面修改（仅保存在本地浏览器）
  </div>
...
async function collabLoadData() {
  try {
    ...
    // 合并新增项目
    if (data.newProjects && data.newProjects.length > 0) {
      const existingIds = new Set(RAW_DATA.allProjects.map(p => p.id));
      data.newProjects.forEach(p => {
        if (!existingIds.has(p.id)) {
          RAW_DATA.allProjects.push(p);
          const dept = p.部门 || '未分配';
          if (!RAW_DATA.depts[dept]) RAW_DATA.depts[dept] = [];
          RAW_DATA.depts[dept].push(p);
...
async function collabCheckUpdates() {
  ...
  try {
    ...
    if (data.lastUpdate && data.lastUpdate !== collabLastUpdate) {
...
function calcHours() {
  ...
  const data = RAW_DATA.allProjects.map(getProject).filter(p => !archived[p.id]);
...
function searchResource() {
  ...
  const personProjects = RAW_DATA.allProjects.filter(p => {
    const actual = localEdits[p.id] || p;
    return actual.资源名称 === person && !archived[p.id];
  });
...
function getFilteredData() {
  ...
  if (currentTab === 'archived') {
    data = data.filter(p => archived[p.id]);
  } else {
    data = data.filter(p => !archived[p.id]);
...
function renderBoardView(data, container) {
  ...
  sortedMarkets.forEach(market => {
    ...
    marketProjects.forEach(p => {
      const projKey = p.项目 + '||' + (p.项目描述 || '');
      if (!byProject[projKey]) byProject[projKey] = [];
      byProject[projKey].push(p);
    });
    ...
    // 按项目排序（有延期的排前面）
    const sortedProjKeys = Object.keys(byProject).sort((a, b) => {
      const aMin = Math.min(...byProject[a].map(p => p._days !== null && p._days !== undefined ? p._days : 999));
      const bMin = Math.min(...byProject[b].map(p => p._days !== null && p._days !== undefined ? p._days : 999));
      return aMin - bMin;
    });

    sortedProjKeys.forEach((projKey, idx) => {
      const resources = byProject[projKey];
      const parts = projKey.split('||');
      const projName = parts[0];
      const projDesc = parts[1] || '';
      ...
      const projDelayed = resources.filter(p => {
        const days = p._days;
        return days !== null && days !== undefined && days <= 3;
      }).length;

      const groupId = 'board_proj_' + market.replace(/[^a-zA-Z0-9]/g, '') + '_' + idx;

      html += '<div class="board-project-group">';
      html += '<div class="board-project-header" onclick="toggleBoardGroup(\\'' + groupId + '\\')">';
      html += '<div>';
      html += '<div class="board-project-title">📁 ' + projName + '</div>';
      if (projDesc) html += '<div class="board-project-desc">' + projDesc.substring(0, 60) + '</div>';
      html += '</div>';
      html += '<div class="board-project-meta">';
      html += '<span class="dept-tag" style="font-size:11px">' + resources.length + ' 项资源</span>';
      ...
      html += '</div>';
      html += '</div>';
      html += '<div id="' + groupId + '" style="display:block">';
...
function saveField(id, field, value) {
  ...
  if (value.trim() === '' || value === '-') {
    ...
    localEdits[id][field] = value.trim();
  }
  localStorage.setItem('projectEdits', JSON.stringify(localEdits));
  updateStats();
  showSaved();
...
function submitNewProject() {
  ...
  // 添加到原始数据
  RAW_DATA.allProjects.push(newProject);
  
  // 更新部门数据
  if (!RAW_DATA.depts[dept]) RAW_DATA.depts[dept] = [];
  RAW_DATA.depts[dept].push(newProject);
