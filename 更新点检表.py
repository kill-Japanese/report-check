  ...
  try {
    ...
    if (users.length === 0) {
      overlay.querySelector('.modal-box').innerHTML = `
        ...
        <div style="text-align:center;padding:40px;color:#999">暂无可选成员</div>
        <div style="margin-top:16px;padding-top:15px;border-top:1px solid #eee;text-align:right">
    ...
    let listHtml = users.map(u => {
      ...
      const emailDisplay = email ? email : '<span style="color:#ccc">未设置邮箱</span>';
      return '<label style="display:flex;align-items:center;padding:8px 10px;border-bottom:1px solid #eee;cursor:pointer;font-size:13px">' +
        ...
        'data-username="' + escapeHtml(u.username) + '" onchange="syncMemberTagSelection(this)">' +
    ...
    overlay.querySelector('.modal-box').innerHTML = `
      ...
      <div style="margin-bottom:10px;padding:8px 12px;background:#e0f2fe;border-radius:6px;font-size:12px;color:#0369a1">
        共 ${users.length} 位成员，点击下方标签或列表勾选，将通过默认邮件客户端发送邮件。
      </div>
    ...
    overlay.querySelector('.modal-box').innerHTML = `
      ...
      <div style="text-align:center;padding:40px;color:red">加载成员列表失败</div>
      <div style="margin-top:16px;padding-top:15px;border-top:1px solid #eee;text-align:right">
        <button class="btn" onclick="document.getElementById('memberSelectModal').remove()">取消</button>
      </div>
    `;
