...
async function batchArchive() {
  ...
  for (const id of ids) {
    const p = RAW_DATA.allProjects.find(x => x.id === id);
    const name = p ? p.项目 : '项目';
    if (collabIsEnabled()) {
      const r = await callActionApi('archive', { id: id });
      if (r && r.success) {
  ...
  updateSelectionCount();
  showSaved();
  alert(`✅ 成功归档 ${success}/${ids.length} 个项目`);
}
...
async function batchDelete() {
  ...
  for (const id of ids) {
    if (collabIsEnabled()) {
      const r = await callActionApi('delete', { id: id });
      if (r && r.success) {
  ...
  updateSelectionCount();
  showSaved();
  alert(`✅ 成功删除 ${success}/${ids.length} 个项目`);
}
